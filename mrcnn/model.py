...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    # Calculate which slices in the mask have non-zero elements
    _idx = np.sum(mask > 0, axis=(0, 1)) > 0
    print("Shape of _idx:", _idx.shape)  # Should be (num_slices,)

    # Use _idx to slice the mask
    mask = mask[:, :, _idx]
    print("New shape of mask:", mask.shape)  # Check the new shape after slicing

    # Generate new class_ids corresponding to the selected slices
    # Ensure that the length of class_ids matches the number of slices in mask
    num_slices = mask.shape[2]
    class_ids = np.arange(1, num_slices + 1)  # Example: creates class_ids like [1, 2, ..., num_slices]

    # Now filter class_ids using the same _idx
    filtered_class_ids = class_ids[_idx]
    print("Filtered class_ids:", filtered_class_ids)  # Check the new shape
    
    bbox = utils.extract_bboxes(mask)

    # Active classes
    # Different datasets have different classes, so track the
    # classes supported in thecl dataset of this image.
...
class MaskRCNN(object):
    ...
    #             * self.config.LOSS_WEIGHTS.get(name, 1.))
    #         # self.keras_model.add_loss(loss)
    #         self.keras_model.add_loss(lambda: tf.add_n(loss))
